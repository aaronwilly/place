...
import { createVideo } from '../../common/api/authApi';
...
const CreateVideoPage = () => {
  ...
  const onUploadApiCall = async (url, file, id, thumbnail) => {
    ...
    axios.put(url, file, { headers: { 'Content-Type': file.type } }).then(async (res) => {
      setIsLoading(false)
      if (res) {
        await createVideo({
          title,
      ...
      <section>
        <div className="row">
          ...
          <div className="col-lg-4 col-sm-12 col-xs-12">
                ...
                <div className="my-3 w-100 my-4 d-flex" style={{ minHeight: '150px', border: '1px dashed #8364E2', maxWidth: 600 }}>
                  <div className="d-flex flex-column m-auto">
                    <div className='m-3 text-center color-7'>
                      ...
                      <span className='color-b fw-semibold'> you can create anything in the support file formats under 5GB MOV, MP4, WMA.</span>
                    </div>
            ...
            {baseFile &&
              <div ref={ref} className='d-flex flex-wrap align-items-center justify-content-center' style={{ width: 260, height: 425 }}>
                <video
                  controls
                  src={baseFile}
                  preload="auto" autoPlay={true}
                  style={{ maxWidth: 260, maxHeight: 425, border: '3px solid brown', borderRadius: 5 }} />
              </div>
            }
            ...
            {baseFile &&
              <div className='d-center'>
                <div className="my-3 mx-auto d-center">
                  <div className="btn-main btn-icon" onClick={() => document.getElementById('file').click()}>
                    <input type="file" id="file" className='d-none' onChange={onChangeFile} />
