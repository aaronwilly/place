...
const AwesomeBtn = ({ reverse, title, link }) => {
  return (
    <StyledWrapper>
      <button
        onClick={() => window.location.href = link}
        className="button"
        style={{ transform: `rotate(${reverse ? -353 : 353}deg) skewX(4deg)` }}
      >
        <div className="bg" />
        ...
        <div className="wrap">
          ...
          <div className="content">
            <span className="char state-1">
              {title.split("").map((item, index) => <span key={index} data-label={item} style={{ "-i": index + 1 }}>{item}</span> )}
            </span>
          </div>
        </div>
      </button>
...
const StyledWrapper = styled.div`
  ...
  .icon div {
    ...
    animation: swingArrow 1s ease"-i"n-out infinite;
    animation-play-state: paused;
  }
  .icon div::before {
