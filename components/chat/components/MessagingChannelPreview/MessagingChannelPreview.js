...
const getAvatarGroup = (members) => {
  if (members.length === 1) {
    return <Avatar image={UtilService.ConvertImg(getCleanImage(members[0]))} name={members[0].user?.id} size={40} />;
  }
...
const MessagingChannelPreview = (props) => {
  ...
  return (
    <div
      ...
      onClick={() => {
        handleRoute(router, '/chat')
        setIsCreating(false)
        setTimeout(() => setActiveChannel(channel), 1000)
      }}
    >
