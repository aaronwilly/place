...
const ItemLeftBox = ({ myNFT, brand, totalSupply, sales, creatorAddress, verificationData, orderDataByTokenId }) => {
  ...
  const [globalRoyalty, setGlobalRoyalty] = useState(0)
  ...
  const ipfsImage = UtilService.ConvertImg2(cImage)
  ...
  useEffect(() => {
    token_address && !UtilService.checkMetasalt(token_address) && onGetRoyalty()
  }, [token_address])
  ...
  return (
    <div>
      ...
      {!isNFTVideo &&
        <picture>
          <img
            src={fullDisplay ? ipfsImage : UtilService.ConvertImg(thumbnail)}
            alt=''
      ...
      {isNFTVideo && <video controls src={ipfsImage} className='w-100'></video>}
      ...
      <div className='offer-title' onClick={() => setIsTitleDescription(!isTitleDescription)}>
        ...
        <span aria-hidden="true" className={`arrow_carrot-${!isTitleDescription ? 'down' : 'up'} fs-24`}></span>
      </div>
      ...
      {isTitleDescription &&
        <div className='offer-body'>
          {myNFT ?
            <div className="mt-2">
              {(description && (UtilService.convertEmoji(description)))}
            </div>
            :
      ...
      {attributes && Array.isArray(attributes) && attributes.length > 0 &&
        <>
          <div className='offer-title' onClick={() => setIsPropertiesDetails(!isPropertiesDetails)}>
            ...
            <span aria-hidden="true" className={`arrow_carrot-${!isPropertiesDetails ? 'down' : 'up'} fs-24`}></span>
          </div>
      ...
      <div className='offer-title' onClick={() => setIsTitleDetails(!isTitleDetails)}>
        ...
        <span aria-hidden="true" className={`arrow_carrot-${!isTitleDetails ? 'down' : 'up'} fs-24`}></span>
      </div>
      ...
      {isTitleDetails && <div className='offer-body'>
        ...
        <div className="w-100 mt-2 d-flex flex-row justify-content-between">
          ...
          {myNFT ?
            <CopyToClipboard
              ...
              <div className="cursor-pointer" title="Copy">{(ercTokenId?.substr(0, 9) + '...' + ercTokenId?.substr(-4))}</div>
            </CopyToClipboard>
            :
            <LoadingSkeleton />
          }
        ...
        <div className="w-100 mt-2 d-flex flex-row justify-content-between">
          <div>Blockchain</div>
          {myNFT ? <div>{'BNB Network'}</div> : <LoadingSkeleton />}
        </div>
        ...
        {type === 'ERC1155' &&
          <div className="w-100 mt-2 d-flex flex-row justify-content-between">
            ...
            {myNFT ? <div>{myNFT?.supply || totalSupply}</div> : <LoadingSkeleton />}
          </div>
        }
