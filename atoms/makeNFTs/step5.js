...
import { createLazyMint, createMintSupplyErc1155 } from '../../common/api/authApi';
...
function Step5({ lazyMintsCount, onFinish }) {
  ...
  const uploadMetadata = async (imageURL) => {
    ...
    const tokenMetadata = {
      ...
      thumbnail,
      category: collection, // will remove later
      name: title,
  ...
  const onMint = async () => {
    ...
    setIsLoading(true);
    const total = lazyMintsCount
    const image = await ipfs.add(file);
    const { cMeta = '', uri: tokenURI = '' } = await uploadMetadata(image);
    const minter = account;
    const tokenId = minter?.toLowerCase() + 'b' + UtilService.FormatNumberLength(chainId.substring(2), 5) + GetRandomIntInRange(10000, 99999) + UtilService.FormatNumberLength(total, 13);
    const supply = counts;
    ...
    try {
      ...
      const metadata = {
        ...
        totalSupply: erc1155 ? Number(supply) : 1,
        privateSale,
        thumbnail,
      }
      ...
      if (isLazyMint) {
        ...
        const res =  await mint.call({ from: minter });
        await mint.send({ from: minter }).on('transactionHash', () => {}).then(() => {})
      ...
      <div className='d-center'>
        ...
        <NftCard
          nft={{
            ...
            price: tokenPrice,
            isVideo,
            thumbnail: UtilService.ConvertImg(thumbnail)
          }}
          disableLink={true}
          big={true}
          favHidden
